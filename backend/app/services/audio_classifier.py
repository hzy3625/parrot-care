...
# 全局模型实例
_model_trainer: Optional[AudioClassifierTrainer] = None
_model_loaded = False


def _get_model_trainer(model_path: Optional[str] = None) -> Optional[AudioClassifierTrainer]:
    """获取或创建模型训练器（单例模式）"""
    ...
    try:
        if model_path is None:
            model_path = str(Path(__file__).parent.parent.parent / "models" / "audio_classifier.pt")
